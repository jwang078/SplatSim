...
# 1. Force a real allocation by doing an operation that prevents lazy zeros
# Adding a small epsilon or just adding 0.0 to a fresh tensor forces storage allocation
means2D = (torch.zeros((N, 3), device=device) + 0.0).requires_grad_(True)

# 2. Ensure it is contiguous in memory
means2D = means2D.contiguous()

# 3. Use an explicit synchronize to ensure the GPU allocator has finished
...
dummy_tensor = torch.zeros((1, 1), device=device, dtype=torch.float32)
torch.cuda.synchronize()
try:
