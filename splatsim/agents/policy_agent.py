...
class DiffusionAgent(Agent):
    ...
    def act(self, obs_dict: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        ee_pos, ee_quat = p.getLinkState(self.dummy_robot, 6)[0], p.getLinkState(self.dummy_robot, 6)[1]
        obs_dict['state'] = np.array([ee_pos[0], ee_pos[1]+0.0, ee_pos[2]])
        ...
        image_2 = image_2 + np.random.normal(0, 0.1, image_2.shape)

        if self.last_image_obs is None:
            self.last_image_obs = image
            self.last_image_obs_1 = image_2
            self.last_state_obs = obs_dict['state'][:2].reshape(1, 1, 2)
            self.last_state_obs = torch.from_numpy(self.last_state_obs).float()



        cur_state_obs = obs_dict['state'][:2].reshape(1, 1, 2)
        ...
        ee_pose = [action_pred[0], action_pred[1], 0.265  ]
        ...
        dummy_joint_pos = p.calculateInverseKinematics(self.dummy_robot, 6, ee_pose , self.correct_ee_quat,
            ...
            lowerLimits=[obs_dict['joint_positions'][k] - np.pi/2 for k in range(6)],
            # upperLimits=[self.initial_joint_state[k] + np.pi/2 for k in range(6)],
            upperLimits=[obs_dict['joint_positions'][k] + np.pi/2 for k in range(6)],
        ...
        if np.linalg.norm(joint_diff) < 0.1:
            self.cur_index += 1
