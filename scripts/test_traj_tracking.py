...
parser.add_argument("--planner_method", type=str, choices=['L-BFGS-B', 'SLSQP'], default="L-BFGS-B", help='Optimization method to use.')
parser.add_argument("--use_cache", action='store_true', help='If set, skip the optimization step and only playback the reference trajectory.')
...
w_collision = 1000.0       # weight for collision penalty
...
def min_clearance(q):
    ...
    for link in joint_ids:
        pts = p.getClosestPoints(bodyA=robot, bodyB=obstacle, distance=search_distance, linkIndexA=link, linkIndexB=-1)
        if pts:
...
if args.scenario == "no_obstacle":
    ...
    obstacle = p.createMultiBody(baseCollisionShapeIndex=box_col,
    ...
    obstacle = p.createMultiBody(baseCollisionShapeIndex=sphere_col,
    ...
    # Goal Orientation (Quaternion: [x, y, z, w])
    goal_quat = p.getQuaternionFromEuler([0, np.pi/2, 0]) # e.g., Roll 90 degrees
elif args.scenario == "small_engine":
...
for j in range(p.getNumJoints(robot)):
    ...
    if ji[2] == p.JOINT_REVOLUTE or ji[2] == p.JOINT_PRISMATIC:
        joint_ids.append(j)
        lower.append(ji[8]); upper.append(ji[9]); vel.append(ji[11]); torque.append(ji[10])
        if len(joint_ids) == ee_joint_number + 1:
            print(f"End effector joint name (index {ee_joint_number}):", ji[1].decode('utf-8'))
...
pos_traj = np.linspace(start_pos, goal_pos, T)

# Use SLERP (Spherical Linear Interpolation) for orientation (Quaternions)
quat_traj = np.array([
    p.getQuaternionSlerp(start_quat, goal_quat, float(i) / (T - 1))
    for i in range(T)
])
...
for i in range(T):
    target_pos = pos_traj[i]
    target_quat = quat_traj[i]
...
p.setRealTimeSimulation(0) # Ensure simulation is not running in real-time during playback
...
def optimization_method():
    ...
    def objective(Q_flat):
        ...
        # tracking cost for only the end effector
        track = np.sum(Q[:, ee_link_index] - q_ref[:, ee_link_index])**2
        # track = np.sum((Q - q_ref)**2)

        # smoothness on velocities
        dQ = np.diff(Q, axis=0) / dt
        smooth = np.sum(dQ**2)

        # collision hinge loss: sum over waypoints
        dists = min_clearance_traj(Q)
        coll = np.sum(hinge(dists)**2)

        cost = w_track*track + w_smooth*smooth + w_collision*coll  
...
if args.use_cache and os.path.exists(opt_path):
    ...
    if args.use_cache:
        print(f"Warning: Cached trajectory not found at {opt_path}. Running optimization instead.")
...
print("python test_playback_traj.py --scenario {args.scenario} --planner_method {args.planner_method} --type opt")
